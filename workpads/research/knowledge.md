...
- I19d remains open for full Crawl4AI-quality readability/markdown and still-richer rendered-page readiness after D281 added source-backed generic page metadata propagation, D282 preserved it across owned browser fallback extraction, D285 aligned agent-facing option docs with the owned extractor surface, D286 preserved code-block whitespace during markdown normalization, D293 added source-backed `crawl4ai.skip_internal_links` markdown option support, D294 added source-backed `crawl4ai.include_sup_sub` markdown option support, D295 added source-backed `crawl4ai.ignore_links` markdown option support, D296 added source-backed `crawl4ai.ignore_images` markdown option support, D297 added source-backed `crawl4ai.ignore_emphasis` markdown option support, D298 added source-backed `crawl4ai.protect_links` markdown option support, D299 added source-backed `crawl4ai.escape_snob` markdown option support, D300 added source-backed `crawl4ai.ignore_mailto_links` markdown option support, D301 added source-backed `crawl4ai.ignore_tables` markdown option support, D302 added source-backed `crawl4ai.bypass_tables` markdown option support, D303 added source-backed `crawl4ai.use_automatic_links` markdown option support, D304 added source-backed `crawl4ai.images_to_alt` markdown option support, D305 added source-backed `crawl4ai.images_as_html` markdown option support, D306 added source-backed `crawl4ai.images_with_size` markdown option support, and D307 added source-backed `crawl4ai.default_image_alt` markdown option support.
- I19e remains open for broader rendered JavaScript parity, manual real logged-in profile/keychain smoke execution, and still-fuller startup/error classification after D284 added source-backed lifecycle/networkidle timeout messages and D287 aligned early Chrome startup exit messages.
...
- Current support-file rule: do not compact `workpads/research/tasks.md`; compact supporting files by moving dense detail to referenced archive files. The largest historical knowledge bundles now route through split archive indexes after D232, the historical session-wrapper PoC spec and implementation plan route through smaller support slices after D243/D263-D265, and the R12/D55-D63/D64-D76/D77-D91/D92-D108/D109-D122/engine-refactor/OAuth-design bundles route through smaller support slices after D268-D280. The current CDP page-script helpers route through behavior-owned modules after D276, Crawl4AI local-content URL compatibility routes through D277, raw/local base-URL option behavior routes through D278, main-content negative class/id exclusion routes through D279, page metadata propagation routes through D281, fallback metadata propagation routes through D282, networkidle reset/timeout parity coverage routes through D283, CDP navigation wait timeout messages route through D284, agent-facing option doc alignment routes through D285, code-block whitespace preservation routes through D286, Chrome early-exit startup message parity routes through D287, browser CDP discovery tests route through behavior-owned modules after D288, mock-site docs-contract tests route through behavior-owned modules after D289, owned extractor option parsing helpers route through a child module after D290, owned page rendering/readiness helpers route through child modules after D291, Aget facade session orchestration routes through a child module after D292, `skip_internal_links` markdown compatibility routes through D293, `include_sup_sub` markdown compatibility routes through D294, `ignore_links` markdown compatibility routes through D295, `ignore_images` markdown compatibility routes through D296, `ignore_emphasis` markdown compatibility routes through D297, `protect_links` markdown compatibility routes through D298, `escape_snob` markdown compatibility routes through D299, `ignore_mailto_links` markdown compatibility routes through D300, `ignore_tables` markdown compatibility routes through D301, `bypass_tables` markdown compatibility routes through D302, `use_automatic_links` markdown compatibility routes through D303, `images_to_alt` markdown compatibility routes through D304, `images_as_html` markdown compatibility routes through D305, `images_with_size` markdown compatibility routes through D306, and `default_image_alt` markdown compatibility routes through D307.
...
| Latest individual decisions | `archive/knowledge/d168-workpad-knowledge-compaction.md` through `archive/knowledge/d307-default-image-alt.md` |
