...
| AgetBrowser boundary | `src/aget_browser.rs`, `src/browser_cdp/`, `src/session/chrome/`, `src/session/login/` | Local browser/CDP/profile/session engine and backend wrapper. Browser CDP tests are split under `src/browser_cdp/tests/` by state conversion, Chrome/profile lifecycle, page scripts, and discovery behavior. Historical implementation detail is archived in `architecture-inputs.md`. |
