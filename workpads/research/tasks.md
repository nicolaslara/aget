...
- Started after D122 made `src/extraction.rs` and `src/browser_cdp.rs` the main agent-context bottlenecks. D123 made the first behavior-preserving extraction split: `src/extraction.rs` became `src/extraction/mod.rs`, and markdown rendering moved to `src/extraction/markdown.rs`. D124 moved the Crawl4AI compatibility process adapter to `src/extraction/command.rs`. D125 moved the legacy `agent-browser` fallback adapter to `src/extraction/fallback_command.rs`. D126 started the CDP split by moving embedded page scripts to `src/browser_cdp/page_scripts.rs`. D127 moved owned HTML cleanup to `src/extraction/html_clean.rs`. D128 moved owned HTTP/static fetch and cookie replay to `src/extraction/http.rs`. D129 moved CDP process helpers to `src/browser_cdp/process.rs`. D130 moved CDP startup/discovery helpers to `src/browser_cdp/discovery.rs`. D131 moved Chrome launch/profile process ownership to `src/browser_cdp/chrome_process.rs`. D132 moved CDP client/session plumbing and cookie/storage translation to `src/browser_cdp/client.rs`. D133 moved extraction artifact, metadata, private-file, and redaction helpers to `src/extraction/artifacts.rs`. D134 moved CDP rendered-page orchestration to `src/browser_cdp/render.rs`. D135 moved Chrome profile state export orchestration to `src/browser_cdp/state.rs`. D136 moved CDP login lifecycle orchestration to `src/browser_cdp/login.rs`. D137 moved the owned extractor execution path to `src/extraction/owned.rs`. D138 moved CDP unit tests to `src/browser_cdp/tests.rs`. D139 moved CDP session data conversion helpers to `src/browser_cdp/session_data.rs`. D140 moved shared mock-site CLI integration helpers to `tests/support/mock_site_cli.rs`. D141 moved mock-site documentation contract tests to `tests/mock_site_docs_contract.rs`. D142 moved mock-site browser/rendering tests to `tests/mock_site_browser.rs`. D143 moved mock-site session/auth tests to `tests/mock_site_sessions.rs`. D144 moved shared session CLI helpers to `tests/support/session_cli.rs`. D145 moved session import and real cmux import tests to `tests/session_cli/imports.rs`. D146 moved session login lifecycle tests to `tests/session_cli/login.rs`. D147 moved shared get CLI helpers to `tests/support/get_cli.rs`. D148 moved session-backed get replay and fallback tests to `tests/get_cli/session.rs`. D149 moved owned mock-site extractor parity coverage to `tests/mock_site_cli/owned.rs`. Remaining work is to split the other large CLI test targets.
