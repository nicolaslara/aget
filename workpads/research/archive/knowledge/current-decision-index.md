...
| D347-D420 individual decision rows | Recent decision routing below |
...
| D400 | `d400-browser-cdp-transport-test-split.md` | Mechanical split of browser CDP transport tests. |
| D401 | `d401-owned-main-content-split.md` | Mechanical split of owned main-content scoring helpers. |
| D402 | `d402-browser-cdp-session-data-split.md` | Mechanical split of browser CDP session-data helpers. |
| D403 | `d403-browser-cdp-discovery-split.md` | Mechanical split of browser CDP discovery helpers. |
| D404 | `d404-html-clean-test-split.md` | Mechanical split of owned HTML cleanup unit tests. |
| D405 | `d405-cmux-import-test-split.md` | Mechanical split of cmux session import tests. |
| D406 | `d406-browser-cdp-client-transport-split.md` | Mechanical split of browser CDP client transport helpers. |
| D407 | `d407-agent-browser-state-filter-split.md` | Mechanical split of agent-browser state filtering helpers. |
| D408 | `d408-main-content-fixture-route-split.md` | Mechanical split of main-content mock-site fixture routes. |
| D409 | `d409-main-binary-helper-split.md` | Mechanical split of main binary argument and envelope helpers. |
| D410 | `d410-extraction-artifact-helper-split.md` | Mechanical split of extraction artifact redaction, metadata, and private-file helpers. |
| D411 | `d411-browser-cdp-navigation-split.md` | Mechanical split of browser CDP navigation, blank-response, lifecycle, and network-idle helpers. |
| D412 | `d412-markdown-dispatch-google-doc-split.md` | Mechanical split of markdown dispatch Google Docs style and list helpers. |
| D413 | `d413-owned-http-helper-split.md` | Mechanical split of owned HTTP source parsing, cookie replay, and local-input tests. |
| D414 | `d414-mock-site-browser-rendering-test-split.md` | Mechanical split of mock-site browser rendering tests by scenario. |
| D415 | `d415-markdown-block-renderer-split.md` | Mechanical split of markdown block rendering helpers by block type. |
| D416 | `d416-browser-cdp-page-session-split.md` | Mechanical split of browser CDP page session helpers by concern. |
| D417 | `d417-extraction-pipeline-helper-split.md` | Mechanical split of extraction pipeline helpers by execution/finalization concern. |
| D418 | `d418-session-cli-command-definition-split.md` | Mechanical split of session CLI command definitions by subcommand family. |
| D419 | `d419-html-clean-url-helper-split.md` | Mechanical split of owned HTML cleanup URL helpers by exported cleanup, element removal, and domain matching concerns. |
| D420 | `d420-cmux-session-import-helper-split.md` | Mechanical split of cmux session import helpers by import, command, domain filtering, and type concerns. |
...
| D168-D420 | `decision-index/` plus per-decision files | Module/test/support decomposition plus owned extractor/browser parity improvements, agent-facing option docs, support-file routing refreshes, login-session injection, markdown options, metadata coverage, CDP diagnostics, transport/readiness coverage, ignored local Chrome smoke refreshes, support-file archive splits, the current no-task-compaction size audit, frame-tree storage export, excluded selector cleanup, consent-popup cleanup, CSS selector option support, cache-mode compatibility options, `keep_attrs` cleanup support, `prettiify` cleaned-HTML formatting, `process_in_browser` local-content routing, explicit request identity via `user_agent`/`locale`/`timezone_id`, the AgetExtractor options/waits parity helper split, the owned page module split, the attached-page capture test split, the markdown normalize helper split, the markdown table renderer split, the markdown writer helper split, the markdown render-dispatch split, the owned option-application split, the owned content extraction split, the inline markdown assertion split, the markdown fixture route split, the get CLI validation test split, the AgetExtractor option validation split, the Aget API support helper split, the markdown link/image assertion split, the cleanup assertion split, the browser render capture split, the get failure test split, the Aget backend facade split, the get session replay test split, the get session fallback test split, the markdown inline renderer split, the session login start test split, the mock backend config split, the browser CDP client page/runtime split, the browser CDP transport test split, the owned main-content scoring split, the browser CDP session-data split, the browser CDP discovery split, the owned HTML cleanup unit-test split, the cmux session import test split, the browser CDP client transport split, the agent-browser state filtering split, the main-content fixture route split, the main binary helper split, the extraction artifact helper split, the browser CDP navigation split, the markdown dispatch Google Docs split, the owned HTTP helper split, the mock-site browser rendering test split, the markdown block renderer split, the browser CDP page session split, the extraction pipeline helper split, the session CLI command definition split, the HTML cleanup URL helper split, and the cmux session import helper split. |
