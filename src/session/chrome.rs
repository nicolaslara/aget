...
pub fn import_chrome_session(options: ChromeImportOptions) -> Result<Session, AgetError> {
    ...
    let result = (|| {
        ...
        let session = read_filtered_session(raw_state.path(), &options, &temp_session)?;
...
fn read_filtered_session(
    ...
    filter_agent_browser_state(state, options, temp_session)
}
...
fn filter_agent_browser_state(
    ...
    for cookie in state.cookies {
        ...
        let session_cookie = SessionCookie {
            ...
            source_session: Some(temp_session.to_string()),
        };
        let key = (
    ...
    for origin in state.origins {
        ...
        if !domain_allowed(&host, &options.domains) {
            continue;
        }
    ...
    let mut session = Session::new(options.name.clone());
    session.source = SessionSource::ChromeProfile {
        profile: options.profile.clone(),
    };
    session.sensitive = true;
    session.allowed_cookie_domains = options.domains.clone();
...
fn unique_raw_state_path(dir: &Path) -> PathBuf {
    let nanos = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|duration| duration.as_nanos())
        .unwrap_or_default();
    dir.join(format!(
        "agent-browser-raw-state-{}-{nanos}.json",
        std::process::id()
    ))
}
...
impl RawStateFile {
    fn new(dir: &Path) -> io::Result<Self> {
        let path = unique_raw_state_path(dir);
...
mod tests {
    ...
    fn filters_cookies_and_origins_by_allowed_domains() {
        let options = ChromeImportOptions {
            profile: "Default".to_string(),
            name: "demo".to_string(),
            domains: vec!["example.com".to_string()],
            tmp_dir: PathBuf::from("unused"),
        };
        let state = AgentBrowserState {
            cookies: vec![
        ...
        let session = filter_agent_browser_state(state, &options, "aget-import-test").unwrap();
    ...
    fn rejects_conflicting_duplicate_cookies() {
        ...
        let error = filter_agent_browser_state(state, &options, "aget-import-test").unwrap_err();
    ...
    fn raw_state_file_uses_private_permissions_and_is_removed_on_drop() {
        ...
        let raw_state = RawStateFile::new(temp.path()).unwrap();
    ...
    fn import_options() -> ChromeImportOptions {
        ChromeImportOptions {
            profile: "Default".to_string(),
            name: "demo".to_string(),
            domains: vec!["example.com".to_string()],
