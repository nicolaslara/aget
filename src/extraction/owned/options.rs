...
pub(crate) fn validate_owned_extraction_options(
    ...
    for option in &options.backend_options {
        ...
        match option_name {
            ...
            "default_image_alt" => {
                owned_options.default_image_alt = option.value.clone();
            }
            "open_quote" => {
                owned_options.open_quote = option.value.clone();
            }
            "close_quote" => {
                owned_options.close_quote = option.value.clone();
            }
            "ul_item_mark" => {
                owned_options.ul_item_mark = option.value.clone();
            }
            "ignore_images" => {
