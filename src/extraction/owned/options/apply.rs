    ...
    match option_name {
        ...
        "locale" => {
            ...
            if locale.is_empty() {
                ...
                owned_options.locale = Some(locale.to_string());
            }
        }
        "timezone_id" => {
            let timezone_id = value.trim();
            if timezone_id.is_empty() {
                owned_options.timezone_id = None;
            } else {
                owned_options.timezone_id = Some(timezone_id.to_string());
            }
        }
        "exclude_all_images" => {
