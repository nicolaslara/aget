...
use super::parse::{
    parse_owned_base_url, parse_owned_body_width, parse_owned_bool, parse_owned_cache_mode,
    parse_owned_excluded_tags, parse_owned_google_list_indent, parse_owned_list,
...
pub(super) fn apply_owned_extractor_option(
    ...
    match option_name {
        ...
        "user_agent" => {
            let user_agent = value.trim();
            if user_agent.is_empty() {
                owned_options.user_agent = None;
            } else {
                owned_options.user_agent = Some(user_agent.to_string());
            }
        }
        "exclude_all_images" => {
