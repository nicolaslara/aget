...
#[test]
fn browser_cdp_render_attached_page_keeps_wait_for_timeout_out_of_image_wait() {
    let listener = TcpListener::bind(("127.0.0.1", 0)).unwrap();
    let port = listener.local_addr().unwrap().port();
    let handle = thread::spawn(move || {
        let (stream, _) = listener.accept().unwrap();
        ...
        reply_ok(
            &mut websocket,
            &request,
            json!({ "result": { "type": "string", "value": "<html><body><main>Images ready</main></body></html>" } }),
        );
    ...
    assert_eq!(
        rendered.html,
        "<html><body><main>Images ready</main></body></html>"
    );
    assert!(rendered.warnings.is_empty());
    handle.join().unwrap();
}
...
#[test]
fn browser_cdp_render_attached_page_reports_runtime_evaluation_exception() {
    let listener = TcpListener::bind(("127.0.0.1", 0)).unwrap();
    let port = listener.local_addr().unwrap().port();
    let handle = thread::spawn(move || {
        let (stream, _) = listener.accept().unwrap();
        ...
        reply_ok(
            ...
            json!({
                ...
                "exceptionDetails": {
                    "text": "Uncaught",
                    "exception": {
                        "description": "Error: location blocked"
                    }
                }
            }),
        );
    ...
    assert!(error
        .to_string()
        .contains("Runtime.evaluate failed: Error: location blocked"));
    handle.join().unwrap();
}
