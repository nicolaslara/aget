...
use self::discovery::{
    classify_chrome_startup_error, connect_existing_profile_browser, wait_for_devtools_active_port,
    wait_for_profile_browser_shutdown,
};
#[cfg(test)]
use self::discovery::{
...
use self::process::{configure_chrome_process_group, ensure_login_browser_exited, terminate_child};
...
impl CdpClient {
    ...
    fn remove_rendered_overlay_elements(
        &mut self,
        session_id: &str,
        timeout: Duration,
    ) -> Result<(), AgetError> {
