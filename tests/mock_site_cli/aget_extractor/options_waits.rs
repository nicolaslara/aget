...
pub(super) fn assert_backend_options_redirects_and_waits(aget_home: &Path, site: &MockSite) {
    ...
    let invalid_cache_mode = Aget::new(aget_home)
        .with_extractor_backend(AgetExtractorBackend::default())
        .get(site.url("/formats"))
        .backend_option("crawl4ai.cache", "sometimes")
        .run()
        .unwrap_err();
    assert_eq!(invalid_cache_mode.code(), ErrorCode::ExtractionFailed);
    assert!(invalid_cache_mode.to_string().contains(
        "crawl4ai.cache supports only bypass, disabled, enabled, read_only, or write_only"
    ));
    ...
    let invalid_handle_code_in_pre = Aget::new(aget_home)
        .with_extractor_backend(AgetExtractorBackend::default())
        .get(site.url("/formats"))
        .backend_option("crawl4ai.handle_code_in_pre", "maybe")
        .run()
        .unwrap_err();
    assert_eq!(
        invalid_handle_code_in_pre.code(),
        ErrorCode::ExtractionFailed
    );
    assert!(invalid_handle_code_in_pre
        .to_string()
        .contains("crawl4ai.handle_code_in_pre expects a boolean value"));

    let invalid_remove_consent_popups = Aget::new(aget_home)
        .with_extractor_backend(AgetExtractorBackend::default())
        .get(site.url("/formats"))
        .backend_option("crawl4ai.remove_consent_popups", "maybe")
        .run()
        .unwrap_err();
    assert_eq!(
        invalid_remove_consent_popups.code(),
        ErrorCode::ExtractionFailed
    );
    assert!(invalid_remove_consent_popups
        .to_string()
        .contains("crawl4ai.remove_consent_popups expects a boolean value"));
    ...
    let invalid_ignore_anchors = Aget::new(aget_home)
        .with_extractor_backend(AgetExtractorBackend::default())
        .get(site.url("/formats"))
        .backend_option("crawl4ai.ignore_anchors", "maybe")
        .run()
        .unwrap_err();
    assert_eq!(invalid_ignore_anchors.code(), ErrorCode::ExtractionFailed);
    assert!(invalid_ignore_anchors
        .to_string()
        .contains("crawl4ai.ignore_anchors expects a boolean value"));
    ...
    let invalid_google_doc = Aget::new(aget_home)
        .with_extractor_backend(AgetExtractorBackend::default())
        .get(site.url("/formats"))
        .backend_option("crawl4ai.google_doc", "maybe")
        .run()
        .unwrap_err();
    assert_eq!(invalid_google_doc.code(), ErrorCode::ExtractionFailed);
    assert!(invalid_google_doc
        .to_string()
        .contains("crawl4ai.google_doc expects a boolean value"));

    let invalid_google_list_indent = Aget::new(aget_home)
        .with_extractor_backend(AgetExtractorBackend::default())
        .get(site.url("/formats"))
        .backend_option("crawl4ai.google_list_indent", "0")
        .run()
        .unwrap_err();
    assert_eq!(
        invalid_google_list_indent.code(),
        ErrorCode::ExtractionFailed
    );
    assert!(invalid_google_list_indent
        .to_string()
        .contains("crawl4ai.google_list_indent expects a positive integer pixel value"));
