...
pub(super) fn assert_document_option_validation(aget_home: &Path, site: &MockSite) {
    ...
    assert_invalid_option_contains(
        aget_home,
        site,
        "crawl4ai.remove_consent_popups",
        "maybe",
        "crawl4ai.remove_consent_popups expects a boolean value",
    );
    assert_invalid_option_contains(
        aget_home,
        site,
        "crawl4ai.base_url",
        "/relative/",
        "crawl4ai.base_url expects an absolute URL",
    );
    assert_invalid_option_contains(
        aget_home,
        site,
