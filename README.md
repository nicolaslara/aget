...
- `--selector`, `--exclude-selector`, `--wait-for-selector`, and repeated `--backend-option backend.key=value` shape extraction. `--wait-for-selector` is CSS-only in v1 for authenticated-session safety: use `css:<selector>` or a plain CSS selector; JavaScript waits are rejected. The owned extractor currently supports `crawl4ai.target_elements`, `crawl4ai.excluded_tags`, `crawl4ai.only_text`, and `crawl4ai.delay_before_return_html` for compatibility with existing callers; unsupported keys fail instead of being ignored.
